		...
		while (xmlNode != null) {
			...
			if ("dataOutputAssociation".equals(nodeName)) {
				...
				if (type != null && type.trim().length() > 0) {
					...
					String mapping = (String) startNode
							.getMetaData("TriggerMapping");
					if (mapping != null) {
...
//				new XPATHAssignmentAction();
				ProcessDialectRegistry.getDialect("xpath");
				
				AssignmentAction xpathAssignmentAction = new XPATHAssignmentAction();
				
				// temporal
				AssignmentAction startAssignmentAction = new AssignmentAction(){
					public void execute(WorkItem workItem,
							ProcessContext context) throws Exception {
						System.err.println("EXECUTE AFTER PROCESS STARTS");
					}
				};

				startNode.setMetaData(ProcessRuntimeImpl.START_ACTION,
				...
				while (subNode instanceof Element) {
					...
					if ("timeCycle".equals(subNodeName)) {
						...
						String language = ((Element) subNode)
						...
						if (period != null && period.trim().length() > 0) {
							...
							if ("int".equals(language)) {
								trigger.setHeader("timer (int:" + period + " "
										+ period + ")");
							} else {
								trigger.setHeader("timer (" + language + ":"
										+ period + ")");
							}
							startNode.addTrigger(trigger);
							break;
	...
	public void writeNode(Node node, StringBuilder xmlDump, int metaDataType) {
		...
		if (triggers != null) {
			...
			if (triggers.size() > 1) {
				throw new IllegalArgumentException(
						"Multiple start triggers not supported");
			}
			...
			if (trigger instanceof ConstraintTrigger) {
				...
				if (constraintTrigger.getHeader() == null) {
					...
					xmlDump.append("        <condition xsi:type=\"tFormalExpression\" language=\""
							+ XmlBPMNProcessDumper.RULE_LANGUAGE
							+ "\">"
					...
					xmlDump.append("        <timeCycle xsi:type=\"tFormalExpression\" language=\""
							+ language + "\">" + cycle + "</timeCycle>" + EOL);
					xmlDump.append("      </timerEventDefinition>" + EOL);
				}
			} else if (trigger instanceof EventTrigger) {
				EventTrigger eventTrigger = (EventTrigger) trigger;
				if (!trigger.getInMappings().isEmpty()) {
					String mapping = eventTrigger.getInMappings().keySet()
							.iterator().next();
					xmlDump.append("      <dataOutput id=\"_"
							+ startNode.getId() + "_Output\" />" + EOL
							+ "      <dataOutputAssociation>" + EOL
							+ "        <sourceRef>_" + startNode.getId()
							+ "_Output</sourceRef>" + EOL
							+ "        <targetRef>" + mapping + "</targetRef>"
							+ EOL + "      </dataOutputAssociation>" + EOL
							+ "      <outputSet>" + EOL
							+ "        <dataOutputRefs>_" + startNode.getId()
							+ "_Output</dataOutputRefs>" + EOL
							+ "      </outputSet>" + EOL);
				}
				String type = ((EventTypeFilter) eventTrigger.getEventFilters()
						.get(0)).getType();
