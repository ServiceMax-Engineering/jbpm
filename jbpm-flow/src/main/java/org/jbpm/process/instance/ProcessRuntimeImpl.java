...
import org.jbpm.process.instance.impl.StartAssignmentAction;
...
public class ProcessRuntimeImpl implements InternalProcessRuntime {
	...
	public ProcessRuntimeImpl(InternalKnowledgeRuntime kruntime) {
		...
		((CompositeClassLoader) getRootClassLoader()).addClassLoader(getClass()
				.getClassLoader());
	...
	public ProcessRuntimeImpl(AbstractWorkingMemory workingMemory) {
		...
		this.kruntime = (InternalKnowledgeRuntime) workingMemory
				.getKnowledgeRuntime();
		((CompositeClassLoader) getRootClassLoader()).addClassLoader(getClass()
				.getClassLoader());
		initProcessInstanceManager();
	...
	private void initProcessInstanceManager() {
		String processInstanceManagerClass = ((SessionConfiguration) kruntime
				.getSessionConfiguration()).getProcessInstanceManagerFactory();
		try {
			processInstanceManager = ((ProcessInstanceManagerFactory) loadClass(
					processInstanceManagerClass).newInstance())
	...
	private void initSignalManager() {
		String signalManagerClass = ((SessionConfiguration) kruntime
				.getSessionConfiguration()).getSignalManagerFactory();
		try {
			signalManager = ((SignalManagerFactory) loadClass(
					signalManagerClass).newInstance())
	...
	private ClassLoader getRootClassLoader() {
		RuleBase ruleBase = ((InternalKnowledgeBase) kruntime
				.getKnowledgeBase()).getRuleBase();
		if (ruleBase != null) {
			return ((InternalRuleBase) ((InternalKnowledgeBase) kruntime
					.getKnowledgeBase()).getRuleBase()).getRootClassLoader();
		}
	...
	public ProcessInstance startProcess(String processId,
		...
		ProcessInstance processInstance = createProcessInstance(processId,
				parameters);
	...
	public ProcessInstance createProcessInstance(String processId,
		...
		try {
			...
			final Process process = kruntime.getKnowledgeBase().getProcess(
					processId);
			if (process == null) {
	...
	public ProcessInstance startProcessInstance(long processInstanceId) {
		try {
			...
			getProcessEventSupport().fireBeforeProcessStarted(processInstance,
					kruntime);
			((org.jbpm.process.instance.ProcessInstance) processInstance)
					.start();
			getProcessEventSupport().fireAfterProcessStarted(processInstance,
					kruntime);
	...
	private org.jbpm.process.instance.ProcessInstance startProcess(
			final Process process, Map<String, Object> parameters) {
		ProcessInstanceFactory conf = ProcessInstanceFactoryRegistry.INSTANCE
				.getProcessInstanceFactory(process);
		if (conf == null) {
			throw new IllegalArgumentException("Illegal process type: "
					+ process.getClass());
		}
		return conf.createProcessInstance(process, kruntime, parameters);
	}
	...
	private void initProcessEventListeners() {
		...
		kruntime.getKnowledgeBase().addEventListener(
				new DefaultKnowledgeBaseEventListener() {
					@Override
					...
					@Override
					public void afterProcessRemoved(
							AfterProcessRemovedEvent event) {
						if (event.getProcess() instanceof RuleFlowProcess) {
							String type = (String) ((RuleFlowProcess) event
									.getProcess()).getMetaData().get(
									"StartProcessEventType");
							StartProcessEventListener listener = (StartProcessEventListener) ((RuleFlowProcess) event
									.getProcess()).getMetaData().get(
									"StartProcessEventListener");
							if (type != null && listener != null) {
								signalManager.removeEventListener(type,
										listener);
							}
						}
					}
				...
				if (triggers != null) {
					for (Trigger trigger : triggers) {
						if (trigger instanceof EventTrigger) {
							final List<EventFilter> filters = ((EventTrigger) trigger)
									.getEventFilters();
							String type = null;
	...
	private void initProcessActivationListener() {
		kruntime.addEventListener(new DefaultAgendaEventListener() {
			public void activationCreated(ActivationCreatedEvent event) {
				String ruleFlowGroup = ((Rule) event.getActivation().getRule())
						.getRuleFlowGroup();
		...
		if (workingMemory != null) {
			workingMemory
					.addEventListener(new org.drools.event.DefaultAgendaEventListener() {
						public void afterRuleFlowGroupDeactivated(
								final RuleFlowGroupDeactivatedEvent event,
								final WorkingMemory workingMemory) {
							signalManager.signalEvent("RuleFlowGroup_"
									+ event.getRuleFlowGroup().getName(), null);
						}
					});
		}
	}
	...
	public void abortProcessInstance(long processInstanceId) {
		...
		if (processInstance == null) {
			throw new IllegalArgumentException(
					"Could not find process instance for id "
							+ processInstanceId);
		}
		((org.jbpm.process.instance.ProcessInstance) processInstance)
